...
def img2patch(img, size, step=1):
    ...
    out_shape = tuple(
        xp.subtract(xp.asarray(img.shape[1: -1]), xp.asarray(size))
        // xp.array(step) + 1
    )
    out_shape = (len(img),) + out_shape + size + (xp.size(img, -1),)
...
def patch2img(x, stride, shape):
    ...
    Returns
    -------
    img : (n_batch, xlen_out, ylen_out, in_channels) ndarray
        image
    """
    img = xp.zeros(shape, dtype=x.dtype)
    kx, ky = x.shape[3: 5]
